...
def minimax(node:Node,depth,upperh,currenth,nodeType):
    if(depth==0 or False):
        return node.gameState.getAIPosition()
    
    if(nodeType):
        maxHeuristic=-10000000
        posibleMovements=node.gameState._showAIMovements()
        for n in posibleMovements:
            newNode=node.expandir(n)

            eval=minimax(newNode,depth-1, upperh,currenth,False)
            alpha=0
